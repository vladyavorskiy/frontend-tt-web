...
export default function GameSettingsModal({ onClose, onStart, showToast, socket }) {
  const [mode, setMode] = useState("solo");
  const [type, setType] = useState("online");
  const [roundTime, setRoundTime] = useState([30, 40, 20]);
  const [wordsPerPlayer, setWordsPerPlayer] = useState(8);
  const [localWordsPerPlayer, setLocalWordsPerPlayer] = useState("8");
  const [localRoundTime, setLocalRoundTime] = useState(["30", "40", "20"]);
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50 p-4">
      <div className="w-full max-w-lg bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="bg-[#00277D] px-6 py-5">
          ...
          <p className="text-gray-300 text-sm mt-1">Задайте параметры перед началом</p>
        </div>
